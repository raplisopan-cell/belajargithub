...
console.log("========== SOAL 14 ==========");
const fetchAllSuppliers = (ids) => Promise.all(ids.map(fetchSuppliersById));

async function getProductsWithSuppliers() {
  try {
    const products = JSON.parse(await fetchProductsFromAPI());
    const suppliers = await fetchAllSuppliers(products.map(p => p.id));
...
getProductsWithSuppliers().then(console.log);